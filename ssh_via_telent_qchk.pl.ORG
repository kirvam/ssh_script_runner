...
foreach my $ii ( 0 .. $#{ $aref } ){
     ...
     _ssh_mk_dir($log,"mkdir",$dir,$user,$pass,$ip);
     sleep 2;
     print "Running: _ssh_scp\n";
     _ssh_scp($log,$cmd,$dir,$user,$pass,$ip,@files);
     sleep 2;
     print "Running: _ssh_cmd\n";
     _ssh_cmd($log,$cmd,$dir,$user,$pass,$ip);
     ###
     #print "$ct: ${$aref}[$ii]\n";
 }
...
sub _ssh_scp {
use Net::SCP::Expect;
my($log,$cmd,$dir,$user,$pass,$host,@files) = @_;
#_ssh_scp($log,$cmd,$dir,$user,$pass,$ip,@files);
...
sub _ssh_mk_dir {
use Net::SSH::Expect;
  my($log,$cmd,$dir,$user,$pass,$host) = @_;
#_ssh_mk_dir($log,$cmd,$dir,$user,$pass,$ip);
#_ssh_mk_dir($log,"mkdir",$dir,$user,$pass,$ip);
...
sub _ssh_cmd {
 my($log,$cmd,$dir,$user,$pass,$host) = @_;
# _ssh_cmd($log,$cmd,$dir,$user,$pass,$ip);
 print "$log,$cmd,$dir,$user,$pass,$host\n";
my $ssh = Net::SSH::Expect->new (
    host => $host,
    password=> $pass,
    user => $user,
...
if( $cat =~ m/(###EC##Y#*)/ig ) {
     ...
     print "## CAT====push to \@failures, $host.",".$resp\n";
     push @failures, $host.",".$resp;
     };
print "---< finished $cmd >---\n";
$ssh->close();
